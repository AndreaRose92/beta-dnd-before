class DndClassSerializer < ActiveModel::Serializer
  ...
  def spell_levels
    ...
    when "Cleric"
      spells = SpellLevel.where(dnd_class_id: 3)
    when "Druid"
      spells = SpellLevel.where(dnd_class_id: 4)
    when "Paladin"
      spells = SpellLevel.where(dnd_class_id: 7)
    when "Ranger"
      spells = SpellLevel.where(dnd_class_id: 8)
    when "Sorcerer"
      spells = SpellLevel.where(dnd_class_id: 10)
    when "Warlock"
      spells = SpellLevel.where(dnd_class_id: 11)
      spells = spells.map{|spell| spell.calculate_warlock_slot}
      spells
    when "Wizard"
      spells = SpellLevel.where(dnd_class_id: 12)
  ...
  def levels
    self.object.dnd_class_levels.map { |lvl|
      ...
      when "Monk"
        level = generic_level.merge(monk_level)
      when "Paladin"
